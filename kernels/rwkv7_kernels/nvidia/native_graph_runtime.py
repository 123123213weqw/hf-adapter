...
class NativeGraphRunner:
    """One fixed-batch CUDA graph bound to the canonical public HF cache.

    The runner owns graph-static physical buffers. Public recurrent tensors are
    always exposed as canonical ``[B,H,K,V]`` views; no native cache subclass,
    graph metadata field, or private cache ABI is required.
    """
    ...
    def __init__(self, owner: Any, packs, batch_size: int) -> None:
        ...
        # backend-v2 publishes the same canonical FP32 cache as the readable
        # HF model. Reduced-precision graph state remains an internal future
        # route and must not leak through the public cache tensor dtype.
        triton_fp16_state_requested = False
    ...
    def _canonical_state_view(self, value: torch.Tensor) -> torch.Tensor:
        if str(getattr(self.state_layout, "value", self.state_layout)) == "kv_v2":
            return self._native_view(value)
        return self._native_view(value).transpose(-1, -2)

    def _cache_bound_to_runner(self, cache: Any) -> bool:
        if len(cache.recurrent_state) != self.num_layers:
            return False
        for layer_index in range(self.num_layers):
            recurrent = cache.recurrent_state[layer_index]
            attention = cache.attention_shift[layer_index]
            ffn = cache.ffn_shift[layer_index]
            if recurrent is None or attention is None or ffn is None:
                return False
            if not _same_tensor_view(
                recurrent, self._canonical_state_view(self.state[layer_index])
            ):
                return False
            if not _same_tensor_view(attention, self._native_view(self.xpa[layer_index])):
                return False
            if not _same_tensor_view(ffn, self._native_view(self.xpf[layer_index])):
                return False
        return True

    def _detach_cache(self, cache: Any) -> None:
        if not self._cache_bound_to_runner(cache):
            return
        for layer_index in range(self.num_layers):
            cache.set_layer(
                layer_index,
                self._canonical_state_view(self.state[layer_index]).clone(),
                self._native_view(self.xpa[layer_index]).clone(),
                self._native_view(self.xpf[layer_index]).clone(),
            )

    def _detach_bound_cache_if_different(self, cache: Any) -> None:
        previous = (
            self._bound_cache_ref() if self._bound_cache_ref is not None else None
        )
        if previous is None or previous is cache:
            return
        self._detach_cache(previous)
        self._bound_cache_ref = None

    def copy_from_cache(self, cache: Any) -> None:
        self.copy_from_cache_calls += 1
        if self._cache_bound_to_runner(cache):
        ...
        for layer_index in range(self.num_layers):
            source_state = convert_recurrent_state_tensor(
                cache.recurrent_state[layer_index],
                "kv_v2",
                self.state_layout,
            )
            self._copy_cache_tensor(self.state[layer_index], source_state)
            self._copy_cache_tensor(
                self.xpa[layer_index], cache.attention_shift[layer_index]
            )
            self._copy_cache_tensor(self.xpf[layer_index], cache.ffn_shift[layer_index])
    ...
    def bind_cache(self, cache: Any) -> None:
        ...
        for layer_index in range(self.num_layers):
            cache.set_layer(
                layer_index,
                self._canonical_state_view(self.state[layer_index]),
                self._native_view(self.xpa[layer_index]),
                self._native_view(self.xpf[layer_index]),
            )
