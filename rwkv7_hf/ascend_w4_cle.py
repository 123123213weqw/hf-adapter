...
def _groupwise_w4_dequant(weight: torch.Tensor, group_size: int) -> torch.Tensor:
    ...
    n, k = weight.shape
    g = weight.reshape(n, k // group_size, group_size).float()
...
def apply_sqrelu_channel_equalization(
    ...
    if key.out_features != value.in_features:
        raise ValueError("key/value intermediate dimensions differ")
    if scale.numel() != key.out_features or bool((scale <= 0).any()):
        raise ValueError("scale must be positive per intermediate channel")
    c = scale.to(device=key.weight.device, dtype=key.weight.dtype)
    root = torch.sqrt(c)
    key.weight.div_(root[:, None])
    if key.bias is not None:
        key.bias.div_(root)
    value.weight.mul_(
        c.to(device=value.weight.device, dtype=value.weight.dtype)[None, :]
    )
...
def calibrate_sqrelu_value_w4(
    ...
    *,
    ...
    scales.extend(
        (float(a), smooth_scale(amax, wmax, alpha=float(a), activation_rms=rms))
        for a in alphas
    )
    candidates = []
    ...
    for alpha, cpu_scale in scales:
        ...
        cos = float(F.cosine_similarity(ref.flatten(), out.flatten(), dim=0))
        candidates.append(
            W4CLECandidate(
                alpha,
                cpu_scale.detach().cpu(),
                mse,
                mse / denom,
                cos,
                float(diff.abs().max()),
