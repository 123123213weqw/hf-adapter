# coding=utf-8
"""Strict opt-in compiled dense FFN route for SM120 native-graph decode.
...
def sm120_compiled_ffn_contract(
    *,
    ...
    return bool(
        ...
        and tuple(int(value) for value in capability) == (12, 0)
    )
...
def prepare_sm120_compiled_ffn(
    ...
    if not sm120_compiled_ffn_contract(
        ...
        raise RuntimeError(
            f"{ENV_NAME}=1 supports only SM120, FP16, B8, 24 layers, and hidden "
