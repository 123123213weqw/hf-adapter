# coding=utf-8
# Ported from rwkv-rs/rwkv7-ascend-npu@b6391271f into the canonical HF adapter.
...
class AscendWeightOnlyLinear(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if bit == 4 and (in_features % group_size or out_features % 8):
            raise ValueError(
                "W4 requires in_features % group_size == 0 and out_features % 8 == 0"
            )
        ...
        self.register_buffer(
            "qweight", torch.empty(0, dtype=torch.int32 if bit == 4 else torch.int8)
        )
        self.register_buffer("scales", torch.empty(0, dtype=torch.float16))
        self.register_buffer(
            "offsets", torch.empty(0, dtype=torch.float16), persistent=bit == 4
        )
        self.register_buffer(
            "bias", torch.empty(0, dtype=torch.float16), persistent=bias
        )
    ...
    def manifest(self) -> QuantManifest:
        return QuantManifest(
            ...
            bias=self.has_bias,
            weight_layout="[K,N] int8"
            if self.bit == 8
            else "[K,N/8] int32, eight signed nibbles",
    ...
    @torch.no_grad()
    def load_fp_weight(
        self, weight: Tensor, bias: Tensor | None = None
        ...
        if self.bit == 8:
            ...
            self.scales = scale.to(
                device=original_device, dtype=torch.float16
            ).contiguous()
            ...
            symmetric_scale = (grouped.abs().amax(dim=2) / 7.0).clamp_min(1e-8)
            use_affine = torch.isfinite(affine_offset) & (
            ...
            q_nk = (
                torch.round(grouped / scale[:, :, None] - offset[:, :, None])
                .clamp(-8, 7)
                .to(torch.int32)
            )
            ...
            self.scales = (
                scale.t().to(device=original_device, dtype=torch.float16).contiguous()
            )
            self.offsets = (
                offset.t().to(device=original_device, dtype=torch.float16).contiguous()
            )
        if self.has_bias:
            ...
            self.bias = (
                bias.detach()
                .to(device=original_device, dtype=torch.float16)
                .contiguous()
            )
    ...
    def from_float(
        ...
        *,
        ...
        return result.load_fp_weight(
            linear.weight.detach(),
            None if linear.bias is None else linear.bias.detach(),
        )
    ...
    def make_npu_fastpath(self, batch: int, *, dtype: torch.dtype = torch.float16):
        ...
        if self.bit == 8:
            ...
            if bias is None:
                ...
                def kernel(x):
                    return op(
                        x, qweight, scales, offsets, None, None, None, group_size, 1
                    )
            else:

                def kernel(x):
                    return (
                        op(x, qweight, scales, offsets, None, None, None, group_size, 1)
                        + bias
                    )
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.qweight.numel() == 0 or self.scales.numel() == 0:
            raise RuntimeError(
                "load_fp_weight or load_state_dict must initialize the module"
            )
...
def save_quantized_linear(
    module: AscendWeightOnlyLinear, directory: str | Path
) -> Path:
...
def load_quantized_linear(
    ...
    *,
    ...
    manifest: dict[str, Any] = json.loads(
        (path / "quant_manifest.json").read_text(encoding="utf-8")
    )
    if manifest.get("format_version") != FORMAT_VERSION:
        raise ValueError(
            f"unsupported quant format version {manifest.get('format_version')}"
        )
    ...
    state = torch.load(
        path / "quantized_linear.pt", map_location=map_location, weights_only=True
    )
...
def _set_quant_submodule(
    root: nn.Module, qualified_name: str, replacement: nn.Module
) -> None:
