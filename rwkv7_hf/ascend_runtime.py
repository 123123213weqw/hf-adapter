...
def allow_unvalidated_ascend() -> bool:
    return (
        os.environ.get("RWKV7_ALLOW_UNVALIDATED_ASCEND", "0").strip().lower() in _TRUE
    )
...
def configure_ascend_defaults(
    *, backend: str = "eager", overwrite: bool = False
) -> dict[str, str]:
...
def enable_ascend(
    ...
    *,
    ...
    if available and not 0 <= index < count:
        raise ValueError(
            f"Ascend device index {index} is outside visible device count {count}"
        )
