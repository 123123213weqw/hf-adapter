...
| PP/TP boundary | **PASS for HF scope** | Two-V100 layer-split `device_map` forward/generate matches the single-device reference; TP checkpoint/head-state partition and PP state-ownership contracts are documented | Native serving-engine TP/PP execution belongs to separate projects and is not an HF release gate |
