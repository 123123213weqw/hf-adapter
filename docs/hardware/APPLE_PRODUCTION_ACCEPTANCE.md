...
| HF PEFT/Trainer/TRL 训练 (`hf_mps_training`) | 9 | 0 | 6 | 0 | 15 |
...
| ✅ PASS | `train.mps.peft_methods` | 常见 PEFT 方法兼容 | LoRA、AdaLoRA、IA3、prefix/prompt tuning 中适用于 CausalLM 的常见配置能初始化、训练、保存和加载；不支持项有明确技术说明。 | 4/4 child proofs pass |
| ⬜ MISSING | `train.mps.state_tuning` | RWKV State Tuning | 提供与官方 time_state 兼容的可训练初始 WKV recurrent state adapter（每层 [H,N,N] FP32；xpa/xpf 保持零初始化，v_first 不持久化）：base 权重冻结，按 batch 安全展开且无别名；0.1B/0.4B/1.5B 在 MPS 上通过有限 loss、非零梯度与更新、Trainer 训练、保存重载、断点续训和 generate continuation，重载后 logits/state/greedy 在声明容差内一致。 | no machine-verifiable proof registered |
...
| ✅ PASS | `train.mps.adapter_roundtrip` | Adapter 保存合并与重载 | PEFT adapter save/load、merge/unmerge 后固定评测 logits/greedy 与训练前后预期一致。 | 2/2 child proofs pass |
