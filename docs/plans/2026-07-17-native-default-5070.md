...
- The official training recipe is pinned to RWKV-LM `e6f74b6` and its `RWKV-v7/train_temp/demo-training-prepare.sh` plus `demo-training-run.sh`. The first script creates the L12/D768/effective-FFN3072 initialization on CPU with B1 and eps `1e-8`; the second trains x070 on Minipile/T512 with B16 BF16, LR `6e-4 -> 6e-5`, betas `0.9/0.99`, eps `1e-18`, weight decay `0.001`, warmup 10, gradient checkpointing, one GPU, DeepSpeed ZeRO-2, and kernel `@rwkv3`. The generic `train.py` summary reports 2688, but pinned fast CMix and the checkpoint are 4x/3072.
