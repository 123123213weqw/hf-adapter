...
Tesla V100 completed the corresponding four-pair matrix on 2026-08-14. It
joins 48 fresh RWKV rows to a SHA-locked 48-row Qwen reference captured on the
same physical server and identical six-package runtime. Raw and
parameter-adjusted Prefill and Decode all pass 48/48. Eight independent
FLA-versus-native 512-token checks and one additional 7.2B/B8 graph closure
also pass; this remains a speed result, not a quality or continuous-E2E claim.
...
Across the Tesla V100 paired P+D table, raw Prefill minimum/median/maximum is
`2.249335x/4.744253x/13.714267x`, and raw Decode is
`1.393444x/2.398393x/4.662333x`. Parameter-adjusted Prefill is
`1.808536x/3.217214x/8.216385x`; parameter-adjusted Decode is
`1.120373x/1.617469x/2.793261x`. All four gates pass all 48 cells. The weakest
adjusted Decode cell is 7.2B/9B B8/P128/D128: RWKV reaches `266.506 tok/s`
against Qwen's `191.257 tok/s`. Eight FLA/native checks plus the targeted
7.2B/B8 closure preserve all 512 greedy tokens and finite logits. Evidence:
[`bench/v100_qwen35_paired_pd_v1_20260814/`](bench/v100_qwen35_paired_pd_v1_20260814/README.md).
The Qwen and RWKV commits differ, and the axes are independently measured; the
claim is matched-shape inference throughput, not quality or continuous E2E.
...
| V100 32GB | Strict 0.4B/0.8B through 7.2B/9B paired P+D, B1/B8, dense FP16 | 48/48 candidate and reference rows; 8/8 FLA/native 512-token checks plus 7.2B/B8 graph closure pass | raw Prefill/Decode minima `2.249335x/1.393444x`; adjusted minima `1.808536x/1.120373x` | **PASS 48/48 + 48/48** |
...
| RTX 5090 | latest g1d/g1i 0.4B/1.5B/2.9B/7.2B vs Qwen3.5, B1/B8, dense FP16 | 24/24 full-FLA references; 8/8 P2048 graph/eager continuation and cache-handoff gates pass | per-cell parameter-adjusted prefill PD min/median `1.072987x/1.317515x`; raw prefill min `1.347871x`; 0.4B/B1/P2048 `61.34k tok/s` | **PASS 24/24** |
...
### V100 strict paired Prefill/Decode v1

The 2026-08-14 strict matrix covers RWKV-7 0.4B/1.5B/2.9B/7.2B versus
Qwen3.5 0.8B/2B/4B/9B at B1/B8, P128/512/2048 and D128/512. All 48 fresh
RWKV rows use Native Graph. Qwen is an immutable, same-server and same-runtime
48-row reference with official FLA operators and StaticCache raw CUDA Graph
Decode; Qwen9 locks math-only SDPA.

All four unrounded per-cell gates pass 48/48. Raw Prefill/Decode minima are
`2.249335x/1.393444x`; parameter-adjusted minima are
`1.808536x/1.120373x`. The weakest adjusted Decode cell is 7.2B/9B
B8/P128/D128. Eight FLA/native 512-token oracles and one targeted 7.2B/B8
native-eager/native-graph closure pass greedy, finite-logits, and cosine
`>=0.9999` gates. This is matched-shape engine throughput, not model quality or
continuous E2E. Evidence:
[`bench/v100_qwen35_paired_pd_v1_20260814/`](bench/v100_qwen35_paired_pd_v1_20260814/README.md).
...
prefill has minimum/median `1.531589x/2.076170x`; parameter-adjusted prefill PD
has minimum/median `1.227477x/1.467758x`. Raw decode has minimum/median
`2.069838x/4.524636x`, while parameter-adjusted decode is
`1.664218x/3.433680x`. The narrowest adjusted cell is 0.4B/0.8B B8/P512:
RWKV reaches `78,949.489 tok/s` versus Qwen `38,534.012 tok/s`, or
`1.227477x` after active-parameter adjustment.

The exact scan tiles and scoped FP16 GEMM-accumulation routes are restricted
to measured RTX 3090 shapes. The separate oracle covers every opted-in direct
or chunk-carried shape: `25/25` prompt and post-prefill cache-handoff rows pass
cosine `>=0.9999` and exact greedy-token gates. Compared with the preceding
artifact, the adjusted-prefill minimum rises from `1.037869x` to `1.227477x`.
This is an inference-throughput result, not a model-quality claim.
...
for 0.4B/1.5B at B1/B2/B4/B8 and 2.9B at B1/B8, prompt
128/512/2048. The 1.5B B1 P512/P2048 rows use exact-card self-chunk and P2048
also uses stacked R/K/V. Other cards and unmeasured shapes retain the compatible
fallback.

The final B1/B8 matrix compares RWKV-7 0.4B/1.5B/2.9B with official Qwen3.5
0.8B/2B/4B. All 36 Qwen rows fail closed unless FLA chunk prefill,
fused-recurrent decode, causal-conv1d prefill/update and fused gated
normalization are live.

| Pair | Batch | Dense prefill min | Dense decode min | Active-work decode min |
|---|---:|---:|---:|---:|
...
for Qwen3.5 0.8B/2B/4B/9B on one RTX 5090. Every row verifies the official
...
is aggregate throughput across eight sequences. Display rounding is
report-only: values at or above 100 tok/s use zero decimals and lower values
use one decimal. Full-precision values and all seven timing samples remain in
the artifact.
...
### Historical Full-FLA Qwen3.5 B1/B8 matrix (2026-07-15)
...
The same-day ceiling follow-up at
[`bench/5090_g1i_qwen35_prefill_pd_sota_20260811/`](bench/5090_g1i_qwen35_prefill_pd_sota_20260811/README.md)
keeps the 24 verified full-FLA reference rows and reruns all candidate cells
after adding recurrent-cache continuation to the native prefill CUDA graph.
It passes 24/24 at adjusted prefill PD minimum/median
`1.072987x/1.317515x`; raw prefill minimum is `1.347871x`. The
0.4B/B1/P2048 candidate rises from `27.27k` to `61.34k tok/s` (`2.2495x`),
and the 8/8 graph-versus-eager P2048 oracle retains prompt/post-handoff cosine
minima `0.99999988/0.99999994` with exact greedy tokens. Removing the negative
7.2B stacked-RKV route reduces its final candidate peak from
`17.4-18.6 GiB` to `14.3-15.5 GiB`.
