...
#   bash bench/runners/run_5090_hf_validation.sh HF_DIR=/workspace/models/rwkv7-g1d-0.1b-hf
set -euo pipefail
...
run python -m py_compile \
  ...
  bench/probes/eval_math500_hf.py \
...
run python bench/probes/eval_math500_hf.py \
  --hf-dir "${HF_DIR}" \
...
run python bench/analyzers/analyze_results.py --results "${RESULTS}" --device "NVIDIA GeForce RTX 5090" --dtype "${DTYPE}" --json \
