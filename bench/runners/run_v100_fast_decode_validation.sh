...
#   COMPONENT_STEPS, FUSED_PROJECTION_STEPS, FUSED_WA_LORA_STEPS, FUSED_WAG_LORA_STEPS, FUSED_WAVG_LORA_STEPS, FUSED_RKV_WAG_PROJECTION_STEPS,
...
FUSED_WAVG_LORA_STEPS="${FUSED_WAVG_LORA_STEPS:-128}"
...
FUSED_RECURRENT_OUTPUT_STEPS="${FUSED_RECURRENT_OUTPUT_STEPS:-128}"
...
run_larger_smoke() {
  ...
  run python bench/probes/bench_larger_model_smoke.py \
    --hf-dir "${hf_dir}" \
...

{
  ...
  run python bench/probes/bench_speed.py \
    --hf-dir "${HF_DIR}" \
    --pth "${PTH}" \
    --backend both \
    --dtype "${DTYPE}" \
  ...
  run python bench/runners/bench_batch_sweep.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
  ...
  run python bench/probes/bench_dynamic_batch.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
  ...
  run python bench/probes/bench_chunked_prefill.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
  ...
  run python bench/probes/bench_quantization.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
  ...
  run python bench/probes/bench_decode_breakdown.py \
    --hf-dir "${HF_DIR}" \
    --pth "${PTH}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
  ...
  run python bench/probes/bench_decode_micro.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
  ...
  run python bench/probes/bench_forward_fast_path.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
  ...
  run python bench/probes/bench_generate_fast_path.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
  ...
  run python bench/probes/bench_fast_token_warmup.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
  ...
  run python bench/runners/bench_native_graph_overhead.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
  ...
  run python bench/probes/bench_native_decode.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --prompt-tokens 32 \
  ...
  run python bench/probes/bench_decode_components.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
  ...
  run python bench/probes/bench_projection_lora.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
  ...
  run python bench/probes/bench_fused_projection.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
  ...
  run python bench/probes/bench_fused_wa_lora.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
  ...
  run python bench/probes/bench_fused_wag_lora.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
  ...
  run python bench/probes/bench_fused_wavg_lora.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
    ...
    --block-k 64 \
    --warmup 8 \
    --steps "${FUSED_WAVG_LORA_STEPS}" \
    --results "${RESULTS}"

  run python bench/probes/bench_fused_rkv_wag_projection.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
  ...
  run python bench/probes/bench_fused_attn_output.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
  ...
  run python bench/probes/bench_fused_ffn.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
  ...
  run python bench/probes/bench_fused_shift_mix.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
  ...
  run python bench/probes/bench_fused_recurrent.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
  ...
  run python bench/probes/bench_fused_recurrent_output.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
    ...
    --layers 0 1 11 \
    --warmup 16 \
    --steps "${FUSED_RECURRENT_OUTPUT_STEPS}" \
    --results "${RESULTS}"

  run python bench/probes/bench_native_graph_fused_recurrent.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
  ...
  for fused_output_bsz in ${NATIVE_GRAPH_FUSED_OUTPUT_BATCH_SIZES}; do
    run python bench/probes/bench_native_graph_fused_recurrent_output.py \
      --hf-dir "${HF_DIR}" \
      --dtype "${DTYPE}" \
      --device "${DEVICE}" \
      --attn-mode fused_recurrent \
      --fuse-norm false \
      --fast-cache true \
      --batch-size "${fused_output_bsz}" \
      --prompt-tokens 64 \
      --warmup 4 \
      --steps "${NATIVE_GRAPH_FUSED_RECURRENT_OUTPUT_STEPS}" \
      --fixed-token \
      --results "${RESULTS}"
  done

  for fused_output_bsz in ${NATIVE_GRAPH_FUSED_OUTPUT_BATCH_SIZES}; do
    run python bench/probes/bench_native_graph_fused_output.py \
      --hf-dir "${HF_DIR}" \
      --dtype "${DTYPE}" \
      --device "${DEVICE}" \
      --attn-mode fused_recurrent \
  ...
  for fused_wavg_bsz in ${NATIVE_GRAPH_FUSED_OUTPUT_BATCH_SIZES}; do
    run python bench/probes/bench_native_graph_fused_wavg_lora.py \
      --hf-dir "${HF_DIR}" \
      --dtype "${DTYPE}" \
      --device "${DEVICE}" \
      --attn-mode fused_recurrent \
  ...
  for rkv_policy_bsz in ${NATIVE_GRAPH_FUSED_OUTPUT_BATCH_SIZES}; do
    run python bench/probes/bench_native_graph_vkwr_rkv_policy.py \
      --hf-dir "${HF_DIR}" \
      --dtype "${DTYPE}" \
      --device "${DEVICE}" \
      --attn-mode fused_recurrent \
  ...
  run python bench/probes/bench_native_quant_gemv.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
  ...
  run python bench/probes/bench_native_quant_w4_gemv.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
  ...
  run python bench/probes/bench_native_quant_rkv.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
  ...
  run python bench/probes/bench_native_quant_w4_rkv.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
  ...
  if should_run_speculative_bench "${RUN_SPECULATIVE_BENCH}" "${SPEC_TARGET_HF_DIR}" "${SPEC_DRAFT_HF_DIR}"; then
    run python bench/probes/bench_speculative_decode.py \
  ...
  run python bench/probes/profile_decode.py \
    --backend hf \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
  ...
  run python bench/analyzers/summarize_results.py \
    --results "${RESULTS}" \
    --device V100 \
    --last 12
  ...
  run python bench/analyzers/analyze_results.py \
    --results "${RESULTS}" \
    --device V100 \
    --dtype "${DTYPE}"

  run python bench/validators/check_results.py \
    --results "${RESULTS}" \
    --device V100 \
    --dtype "${DTYPE}"
