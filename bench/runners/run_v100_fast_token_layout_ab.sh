...
for layout in ${LAYOUTS}; do
  ...
  RWKV7_FAST_TOKEN_LAYOUT=${layout} \
  python bench/probes/bench_decode_micro.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
...
python bench/analyzers/compare_fast_token_layouts.py --results "${RESULTS}" --device V100 --dtype "${DTYPE}" --require-candidate --min-speedup 1.0
python bench/analyzers/analyze_results.py --results "${RESULTS}" --device V100 --dtype "${DTYPE}"
python bench/validators/check_results.py --results "${RESULTS}" --device V100 --dtype "${DTYPE}"
