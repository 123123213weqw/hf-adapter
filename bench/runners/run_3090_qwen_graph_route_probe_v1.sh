...
"${PYTHON_BIN}" "${ROOT}/bench/validators/check_exact_gpu.py" --model 3090 --name "${gpu_name}"
...
run_route() {
  ...
  XDG_CACHE_HOME="${cache}/xdg" TORCHINDUCTOR_CACHE_DIR="${cache}/inductor" \
    ...
    "${ROOT}/bench/runners/bench_cross_model_speed_resident.py" \
...
"${PYTHON_BIN}" "${ROOT}/bench/analyzers/select_qwen35_graph_route_v1.py" \
