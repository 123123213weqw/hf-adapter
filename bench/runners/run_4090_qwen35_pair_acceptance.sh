...
if [[ "${ALLOW_NON_4090:-0}" != "1" ]] && ! "${PYTHON_BIN}" \
  "${ROOT}/bench/validators/check_exact_gpu.py" --model "${REQUIRED_GPU_MODEL}" --name "${gpu_name}"; then
...
"${PYTHON_BIN}" bench/analyzers/compose_qwen35_quant_routes.py \
...
"${PYTHON_BIN}" bench/analyzers/compare_qwen35_speed_matrix.py \
