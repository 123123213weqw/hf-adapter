...
"${PYTHON_BIN}" "${ROOT}/bench/validators/check_exact_gpu.py" --model "${GPU_MODEL}" --name "${gpu_name}"
...
"${PYTHON_BIN}" "${ROOT}/bench/tools/capture_hf_fast_path_v1_environment.py" \
...
"${PYTHON_BIN}" bench/validators/validate_hf_fast_path_v1.py \
  --candidate-results \
