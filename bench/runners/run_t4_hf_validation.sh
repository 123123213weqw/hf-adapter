...
if [[ "${MATRIX_MODE}" == "full" && "${T4_CHILD_RUN}" != "1" ]]; then
  ...
  if [[ "${RUN_ANALYZE}" != "0" && -s "${RESULTS}" ]]; then
    "${PYTHON_BIN}" bench/analyzers/analyze_results.py \
...
run "${PYTHON_BIN}" -m py_compile \
  ...
  bench/runners/bench_batch_sweep.py \
  bench/runners/bench_native_graph_overhead.py \
  bench/runners/bench_native_prefill_scan.py \
  bench/probes/bench_chunked_prefill.py \
  bench/probes/bench_native_graph_fused_output.py \
  bench/probes/bench_native_graph_fused_recurrent_output.py \
  bench/probes/bench_quantization.py \
  bench/runners/bench_native_mm_quant_decode.py \
  bench/runners/bench_native_quant_e2e_decode.py \
...
if [[ "${RUN_PERF}" != "0" ]]; then
  run "${PYTHON_BIN}" bench/runners/bench_batch_sweep.py \
  ...
  run "${PYTHON_BIN}" bench/runners/bench_native_graph_overhead.py \
...
if [[ "${RUN_LONG_PREFILL}" != "0" ]]; then
  run "${PYTHON_BIN}" bench/probes/bench_chunked_prefill.py \
    --hf-dir "${HF_DIR}" --dtype "${DTYPE}" --device "${DEVICE}" \
...
if [[ "${RUN_PREFILL}" != "0" ]]; then
  ...
  for fused_scan in false true; do
    run "${PYTHON_BIN}" bench/runners/bench_native_prefill_scan.py \
      --model "${HF_DIR}" --device "${DEVICE}" --dtype "${DTYPE}" \
...
if [[ "${RUN_FUSED_AB}" != "0" ]]; then
  for batch_size in ${BATCH_SIZES}; do
    run "${PYTHON_BIN}" bench/probes/bench_native_graph_fused_output.py \
      --hf-dir "${HF_DIR}" --dtype "${DTYPE}" --device "${DEVICE}" \
      --attn-mode fused_recurrent --fuse-norm false --fast-cache true \
      --batch-size "${batch_size}" --prompt-tokens "${PROMPT_TOKENS}" \
      --warmup "${WARMUP}" --steps "${STEPS}" --results "${RESULTS}" \
      | tee "${OUT_DIR}/fused_output_bsz${batch_size}.log"
    run "${PYTHON_BIN}" bench/probes/bench_native_graph_fused_recurrent_output.py \
      --hf-dir "${HF_DIR}" --dtype "${DTYPE}" --device "${DEVICE}" \
      --attn-mode fused_recurrent --fuse-norm false --fast-cache true \
      --batch-size "${batch_size}" --prompt-tokens "${PROMPT_TOKENS}" \
      --warmup "${WARMUP}" --steps "${STEPS}" --results "${RESULTS}" \
...
if [[ "${RUN_QUANT}" != "0" ]]; then
  ...
  run env RWKV7_NATIVE_MODEL=1 "${PYTHON_BIN}" bench/probes/bench_quantization.py \
    --hf-dir "${HF_DIR}" --model-size-label "${MODEL_SIZE_LABEL}" \
...
if [[ "${RUN_NATIVE_MM_QUANT}" != "0" ]]; then
  for policy in memory speed; do
    run "${PYTHON_BIN}" bench/runners/bench_native_mm_quant_decode.py \
      --hf-dir "${HF_DIR}" --model-size-label "${MODEL_SIZE_LABEL}" \
      --dtype "${DTYPE}" --device "${DEVICE}" \
  ...
  run "${PYTHON_BIN}" bench/runners/bench_native_quant_e2e_decode.py \
    --hf-dir "${HF_DIR}" --model-size-label "${MODEL_SIZE_LABEL}" \
    --dtype "${DTYPE}" --device "${DEVICE}" --attn-mode fused_recurrent \
...
if [[ "${RUN_ANALYZE}" != "0" && -s "${RESULTS}" ]]; then
  run "${PYTHON_BIN}" bench/analyzers/analyze_results.py \
    --results "${RESULTS}" --device "${EXPECTED_GPU_NAME}" --dtype "${DTYPE}" --json \
    > "${RESULTS%.jsonl}.report.json"
else
