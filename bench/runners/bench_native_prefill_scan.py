...
from bench.benchlib.model_loader import load_causal_lm, load_tokenizer
from bench.benchlib.results import append_jsonl
from bench.benchlib.timing import measure_ms, median, temporary_environ
...
def measured_ms(fn: Callable[[], Any], device: str, timing: str) -> float:
    ...
    return measure_ms(fn, device, mode=timing)


env_override = temporary_environ
...
def run_case(args: argparse.Namespace, tok, model, batch_size: int, prompt_tokens: int) -> dict[str, Any]:
    ...
    reference_env = (
        ...
        if args.reference_backend == "hf"
            ...
            "RWKV7_NATIVE_PREFILL_FUSED_STATE_PREP": "0",
            "RWKV7_NATIVE_PREFILL_FUSED_STATE_SCAN": "0",
            "RWKV7_NATIVE_PREFILL_FUSED_OUTPUT": "0",
...
def main() -> int:
    ...
    try:
        tok = load_tokenizer(effective_model_path)
        ...
        model = load_causal_lm(effective_model_path, **load_kwargs)
