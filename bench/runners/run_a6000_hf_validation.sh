...
#
# Examples:
#   bash bench/runners/run_a6000_hf_validation.sh
#   RUN_CONVERT=0 RUN_TRAINING=0 RUN_ZERO=0 bash bench/runners/run_a6000_hf_validation.sh
set -euo pipefail
...
run_batch_sweep_one() {
  ...
  run_single_gpu "${PYTHON_BIN}" bench/runners/bench_batch_sweep.py \
    --hf-dir "${model}" \
...
run_native_mm_quant_one() {
  ...
  run_single_gpu "${PYTHON_BIN}" bench/runners/bench_native_mm_quant_decode.py \
    --hf-dir "${model}" \
    --model-size-label "${label}" \
    --device cuda \
...
run_quant_one() {
  ...
  run_single_gpu "${PYTHON_BIN}" bench/probes/bench_quantization.py \
    --hf-dir "${model}" \
    --model-size-label "${label}" \
    --device cuda \
    --dtype "${QUANT_DTYPE}" \
...
run_01b_baseline() {
  ...
  run_single_gpu "${PYTHON_BIN}" tests/test_quantized_inference.py \
    ...
    --quantization 4bit \
    --optional
  run_single_gpu "${PYTHON_BIN}" bench/probes/bench_speed.py \
    --hf-dir "${model}" \
    --model-size-label "${label}" \
    --pth "${pth}" \
    --backend hf \
...
run_larger_smoke_one() {
  ...
  run_single_gpu "${PYTHON_BIN}" bench/probes/bench_larger_model_smoke.py \
    --hf-dir "${model}" \
    --model-size-label "${label}" \
    --checkpoint-path "${pth}" \
    --device cuda \
