...
#   bash bench/runners/run_4090_fused_backend_validation.sh \
...
run python -m py_compile \
  ...
  bench/probes/bench_native_graph_fused_wavg_lora.py \
  bench/probes/bench_native_graph_fused_projection.py \
  bench/probes/bench_native_graph_fused_output_project.py \
...
if [[ "${RUN_NEGATIVE_PROBES}" != "0" ]]; then
  ...
  run python bench/probes/bench_native_graph_fused_projection.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
  ...
  run python bench/probes/bench_native_graph_fused_output_project.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
...
run python bench/analyzers/analyze_results.py --results "${RESULTS}" --device "NVIDIA GeForce RTX 4090" --dtype "${DTYPE}" --json > "${RESULTS%.jsonl}.report.json"
echo "wrote ${RESULTS}"
