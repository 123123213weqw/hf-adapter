...
This directory preserves the **2026-07-16 V100 acceptance snapshot** already
committed to the repository. It does not replace newer V100 evidence or the
current conclusions in `HF_STATUS.md`, `BENCHMARK.md`, and
`docs/HARDWARE_MATRIX.md`. The fail-closed summarizer reads the dated
production-close bundle, the two-cell full-FLA Qwen comparison, and the
historical torch-fallback Qwen matrix with their correct, separate contracts.

## Snapshot verdict

Evidence validation for this dated snapshot is **PASS**. At the snapshot date,
overall V100 work remained **PARTIAL** because the measured lanes did not cover
every model, shape, quantization path, quality axis, or Albatross tier. Later
packed-MM4 and prefill work is intentionally evaluated by newer artifacts.
...
| Full-memory native MM8/MM4 | OPEN at snapshot date | Historical PR #21 was not promoted: MM4 had greedy mismatches and MM8 missed every speed cell |
...
the backend pinned to `torch`, the repository comparator passes with minimum
...
Qwen3.5 and current post-2026-07-16 V100 status are not claimed.
