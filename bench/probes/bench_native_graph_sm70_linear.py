#!/usr/bin/env python3
# coding=utf-8
"""End-to-end A/B for the measured Volta small-row fp16 linear routes.

The benchmark changes only ``RWKV7_NATIVE_GRAPH_SM70_LINEAR`` between
captures. Existing output, recurrent-output, and norm/mix fusions remain
enabled in both modes, so the result measures the incremental decode benefit.
...
def run_mode(model, token: torch.Tensor, base_state, args: argparse.Namespace, *, enabled: bool) -> dict[str, Any]:
    os.environ["RWKV7_NATIVE_GRAPH_SM70_LINEAR"] = "1" if enabled else "0"
...
def main() -> int:
    ...
    with torch.inference_mode():
        os.environ["RWKV7_NATIVE_GRAPH_SM70_LINEAR"] = "0"
        os.environ["RWKV7_NATIVE_GRAPH_FUSED_NORM_MIX"] = "1"
        os.environ["RWKV7_NATIVE_GRAPH_FUSED_WAVG_LORA"] = "0"
        os.environ["RWKV7_NATIVE_GRAPH_RKV_POLICY"] = "off"
