#!/usr/bin/env python3
# coding=utf-8
"""End-to-end A/B for the RTX 4090 no-copy exact-row linear route.

The benchmark changes only ``RWKV7_NATIVE_GRAPH_ADA_LINEAR`` between captures.
Sparse FFN stays disabled, while output/recurrent-output, raw recurrent, and
decode norm/mix routes remain enabled in both modes. It records first-step
logits, a multi-step greedy window, graph-cache telemetry, latency, throughput,
and peak memory.
"""
...
def run_mode(model, token: torch.Tensor, base_state, args: argparse.Namespace, *, enabled: bool) -> dict[str, Any]:
    os.environ["RWKV7_NATIVE_GRAPH_ADA_LINEAR"] = "1" if enabled else "0"
