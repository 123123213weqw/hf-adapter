#!/usr/bin/env python3
# coding=utf-8
"""Benchmark native fused row-wise W4 R/K/V dequant-GEMV prototype.

This measures the launch-fusion step missing from per-module W4 GEMV telemetry:
three decode-hot RWKV attention projections are computed in one Triton launch
and compared with both fp16 linears and three separate native W4 GEMVs.
"""
...
def separate_int4_rkv(qpack: dict[str, Any], xs: dict[str, torch.Tensor], block_m: int, block_k: int):
    return (
        int4_rowwise_gemv(xs["xr"], qpack["qr"], qpack["sr"], block_m=block_m, block_k=block_k),
        int4_rowwise_gemv(xs["xk"], qpack["qk"], qpack["sk"], block_m=block_m, block_k=block_k),
        int4_rowwise_gemv(xs["xv"], qpack["qv"], qpack["sv"], block_m=block_m, block_k=block_k),
    )


def fused_int4_rkv(qpack: dict[str, Any], xs: dict[str, torch.Tensor], block_m: int, block_k: int):
...
def qpack_footprint_bytes(qpack: dict[str, Any]) -> int:
    return (
        int4_weight_footprint_bytes(qpack["qr"], qpack["sr"])
        + int4_weight_footprint_bytes(qpack["qk"], qpack["sk"])
        + int4_weight_footprint_bytes(qpack["qv"], qpack["sv"])
    )
...
def main() -> int:
    ...
    for layer_idx in args.layers:
        ...
        int4_bytes = qpack_footprint_bytes(qpack)
        with torch.inference_mode():
            fp16_out = current_fp16_rkv(attn, xs)
            separate_out = separate_int4_rkv(qpack, xs, args.block_m, args.block_k)
            fused_out = fused_int4_rkv(qpack, xs, args.block_m, args.block_k)
        fp16_ms = timed(lambda: current_fp16_rkv(attn, xs), args.device, args.warmup, args.steps)
        separate_ms = timed(lambda: separate_int4_rkv(qpack, xs, args.block_m, args.block_k), args.device, args.warmup, args.steps)
        fused_ms = timed(lambda: fused_int4_rkv(qpack, xs, args.block_m, args.block_k), args.device, args.warmup, args.steps)
        rows.append(
            {
                "layer_idx": layer_idx,
                "fp16_current_ms": round(fp16_ms, 5),
                "separate_int4_ms": round(separate_ms, 5),
                "fused_int4_ms": round(fused_ms, 5),
                "fused_speedup_vs_fp16": round(fp16_ms / fused_ms, 4) if fused_ms else None,
                "fused_speedup_vs_separate_int4": round(separate_ms / fused_ms, 4) if fused_ms else None,
    ...
    avg_separate = sum(float(r["separate_int4_ms"]) for r in rows) / len(rows)
    avg_fused = sum(float(r["fused_int4_ms"]) for r in rows) / len(rows)
    total_fp16_mb = sum(float(r["fp16_weight_mb"]) for r in rows)
    total_int4_mb = sum(float(r["int4_weight_mb"]) for r in rows)
    row = {
        "axis": "native_quant_w4_rkv_proto",
        "backend": "hf_adapter",
        "prototype_backend": "triton_int4_fused_rkv_gemv" if native_int4_fused_rkv_available() else "torch_fallback",
        "status": "pass",
        ...
        "avg_separate_int4_ms": round(avg_separate, 5),
        "avg_fused_int4_ms": round(avg_fused, 5),
        "fused_speedup_vs_fp16": round(avg_fp16 / avg_fused, 4) if avg_fused else None,
        "fused_speedup_vs_separate_int4": round(avg_separate / avg_fused, 4) if avg_fused else None,
