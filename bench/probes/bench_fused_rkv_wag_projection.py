#!/usr/bin/env python3
# coding=utf-8
"""Benchmark combined fused fp16 R/K/V + W/A/G LoRA projection prototype."""
...
from rwkv7_hf.fused_attention_projection import fused_rkv_wag_projection, fused_rkv_wag_projection_available
...
def current_rkv_wag(attn, xs: dict[str, torch.Tensor]):
    return (
        attn.r_proj(xs["xr"]),
        attn.k_proj(xs["xk"]),
        attn.v_proj(xs["xv"]),
...
def prototype_rkv_wag(attn, xs: dict[str, torch.Tensor], block_m: int, block_r: int, block_k: int):
    return fused_rkv_wag_projection(
        xs["xr"],
        xs["xk"],
        xs["xv"],
        xs["xw"],
...
def main() -> int:
    ...
    for layer_idx in args.layers:
        ...
        with torch.inference_mode():
            cur = current_rkv_wag(attn, xs)
            proto = prototype_rkv_wag(attn, xs, args.block_m, args.block_r, args.block_k)
        current_ms = timed(lambda: current_rkv_wag(attn, xs), args.device, args.warmup, args.steps)
        prototype_ms = timed(lambda: prototype_rkv_wag(attn, xs, args.block_m, args.block_r, args.block_k), args.device, args.warmup, args.steps)
        layer_rows.append(
            {
                "layer_idx": layer_idx,
                "ranks": layer_ranks,
    ...
    row = {
        ...
        "backend": "hf_adapter",
        "prototype_backend": "triton_rkv_wag_down_plus_wag_up" if fused_rkv_wag_projection_available() else "torch_fallback",
        "status": "pass",
        "dtype": args.dtype,
