#!/usr/bin/env python3
# coding=utf-8
"""A/B benchmark for native_graph with fused output-prep + o_proj enabled.

`bench_fused_attn_output_project.py` proves the isolated one-kernel
output-prep-plus-o_proj prototype can be fast. This script checks the
production-facing question: after capture inside the HF native_graph fast-token
backend, does enabling `RWKV7_NATIVE_GRAPH_FUSED_OUTPUT_PROJECT=1` preserve
logits/greedy behavior and move end-to-end decode latency on top of the current
default output-prep fusion?
"""
...
def run_mode(model, token: torch.Tensor, base_state, args: argparse.Namespace, *, enabled: bool) -> dict[str, Any]:
    ...
    os.environ["RWKV7_NATIVE_GRAPH_FUSED_OUTPUT_PROJECT"] = "1" if enabled else "0"
...
def main() -> int:
    ...
    ap.add_argument("--fused-output", action=argparse.BooleanOptionalAction, default=True, help="Keep default fused output prep enabled in baseline mode; project mode overrides it with output+o_proj fusion.")
    ap.add_argument("--fused-projection", action="store_true", help="Keep RWKV7_NATIVE_GRAPH_FUSED_PROJECTION=1 in both A/B modes to test combined projection/output-project integration.")
    ap.add_argument("--fused-recurrent", action="store_true", help="Keep RWKV7_NATIVE_GRAPH_FUSED_RECURRENT=1 in both A/B modes to test combined recurrent/output-project integration.")
    ap.add_argument("--block-m", type=int, default=16, help="Output row tile for RWKV7_NATIVE_GRAPH_FUSED_OUTPUT_PROJECT_BLOCK_M.")
    ...
    row = {
        ...
        "fused_projection_enabled": bool(args.fused_projection),
        "output_project_block_m": int(args.block_m),
