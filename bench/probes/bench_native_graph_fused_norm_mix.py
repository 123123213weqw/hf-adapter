#!/usr/bin/env python3
# coding=utf-8
"""End-to-end A/B for decode layer-norm/residual/time-mix fusion.

The benchmark changes only ``RWKV7_NATIVE_GRAPH_FUSED_NORM_MIX`` between
captures. Existing output/recurrent-output fusions remain enabled in both
modes, so the result measures the incremental production decode benefit.
...
def run_mode(model, token: torch.Tensor, base_state, args: argparse.Namespace, *, enabled: bool) -> dict[str, Any]:
    os.environ["RWKV7_NATIVE_GRAPH_FUSED_NORM_MIX"] = "1" if enabled else "0"
...
def main() -> int:
    ...
    row = {
        "axis": "native_graph_fused_decode_norm_mix",
        "backend": "hf_adapter",
