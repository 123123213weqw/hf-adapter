#!/usr/bin/env python3
# coding=utf-8
"""End-to-end A/B for the RTX 4090 small-row sparse FFN route.

The benchmark changes only ``RWKV7_NATIVE_GRAPH_ADA_SPARSE_FFN`` between
captures. Promoted output/recurrent-output fusions plus the measured raw
recurrent and decode norm/mix routes remain enabled in both modes. It records
first-step logits, a multi-step greedy window, graph-cache telemetry, latency,
throughput, and the packed-value memory cost.
"""
...
def run_mode(model, token: torch.Tensor, base_state, args: argparse.Namespace, *, enabled: bool) -> dict[str, Any]:
    os.environ["RWKV7_NATIVE_GRAPH_ADA_SPARSE_FFN"] = "1" if enabled else "0"
...
def main() -> int:
    ...
    row = {
        "axis": "native_graph_ada_sparse_ffn",
        "backend": "hf_adapter",
