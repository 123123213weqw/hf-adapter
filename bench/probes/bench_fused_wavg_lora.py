#!/usr/bin/env python3
# coding=utf-8
"""Benchmark optional fused fp16 W/A/G LoRA + V-gate projection prototype."""
...
from rwkv7_hf.fused_lora import fused_wavg_lora, fused_wavg_lora_available
...
def _bias(module) -> torch.Tensor | None:
    return getattr(module.lora[2], "bias", None)


def current_wavg(attn, xs: dict[str, torch.Tensor]):
    if getattr(attn, "v_lora", None) is None:
        raise ValueError("fused W/A/G/V-gate benchmark requires a layer with v_lora; skip layer 0")
    return (
        attn.w_lora(xs["xw"]),
        attn.a_lora(xs["xa"]),
        attn.g_lora(xs["xg"]),
        attn.v_lora(xs["xv"]).sigmoid(),
    )


def prototype_wavg(attn, xs: dict[str, torch.Tensor], block_m: int, block_r: int, block_k: int):
    if getattr(attn, "v_lora", None) is None:
        raise ValueError("fused W/A/G/V-gate benchmark requires a layer with v_lora; skip layer 0")
    return fused_wavg_lora(
        xs["xw"],
        xs["xa"],
        xs["xg"],
        xs["xv"],
        ...
        attn.g_lora.lora[0].weight,
        attn.v_lora.lora[0].weight,
        attn.w_lora.lora[2].weight,
        attn.a_lora.lora[2].weight,
        attn.g_lora.lora[2].weight,
        attn.v_lora.lora[2].weight,
        _bias(attn.w_lora),
        _bias(attn.a_lora),
        _bias(attn.g_lora),
        _bias(attn.v_lora),
...
def main() -> int:
    ...
    ap.add_argument("--layers", nargs="+", type=int, default=[1, 11])
    ...
    for layer_idx in args.layers:
        ...
        layer_ranks = {
            "w": int(attn.w_lora.lora[0].weight.shape[0]),
            "a": int(attn.a_lora.lora[0].weight.shape[0]),
            "g": int(attn.g_lora.lora[0].weight.shape[0]),
            "v": int(attn.v_lora.lora[0].weight.shape[0]) if getattr(attn, "v_lora", None) is not None else 0,
        }
        ranks.append(layer_ranks)
        with torch.inference_mode():
            cur = current_wavg(attn, xs)
            proto = prototype_wavg(attn, xs, args.block_m, args.block_r, args.block_k)
        current_ms = timed(lambda: current_wavg(attn, xs), args.device, args.warmup, args.steps)
        prototype_ms = timed(lambda: prototype_wavg(attn, xs, args.block_m, args.block_r, args.block_k), args.device, args.warmup, args.steps)
        layer_rows.append(
            {
                "layer_idx": layer_idx,
                "ranks": layer_ranks,
    ...
    row = {
        ...
        "backend": "hf_adapter",
        "prototype_backend": "triton_fused_wavg_lora" if fused_wavg_lora_available() else "torch_fallback",
        "status": "pass",
        "dtype": args.dtype,
