...
def run_mode(model, token: torch.Tensor, base_state, args: argparse.Namespace, *, enabled: bool) -> dict[str, Any]:
    ...
    os.environ["RWKV7_NATIVE_GRAPH_FUSED_PROJECTION"] = "1" if enabled else "0"
...
def main() -> int:
    ...
    ap.add_argument("--fused-recurrent", action="store_true", help="Keep RWKV7_NATIVE_GRAPH_FUSED_RECURRENT=1 in both A/B modes to test combined projection integration.")
