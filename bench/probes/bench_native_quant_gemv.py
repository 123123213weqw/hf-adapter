#!/usr/bin/env python3
# coding=utf-8
"""Benchmark native row-wise W8 dequant-GEMV prototypes for RWKV-7 linears.

This is the first RWKV-native quantization serving prototype. It compares the
current fp16 linear calls with a row-wise int8 packed weight plus fused dequant
...
def main() -> int:
    ...
    for layer_idx in args.layers:
        ...
        for module_name in args.modules:
            ...
            q_weight, scales = quantize_int8_rowwise(weight)
            ...
            with torch.inference_mode():
                ...
                proto = int8_rowwise_gemv(x, q_weight, scales, bias, block_m=args.block_m, block_k=args.block_k)
            current_ms = timed(lambda: F.linear(x, weight, bias), args.device, args.warmup, args.steps)
            prototype_ms = timed(
                lambda: int8_rowwise_gemv(x, q_weight, scales, bias, block_m=args.block_m, block_k=args.block_k),
                args.device,
                args.warmup,
                args.steps,
            )
            fp16_bytes = int(weight.numel()) * int(weight.element_size()) + (int(bias.numel()) * int(bias.element_size()) if bias is not None else 0)
            int8_bytes = int8_weight_footprint_bytes(q_weight, scales, bias)
            rows.append(
                {
                    "layer_idx": layer_idx,
                    "module": module_name,
    ...
    total_int8_mb = sum(float(r["int8_weight_mb"]) for r in pass_rows)
    row = {
        "axis": "native_quant_gemv_proto",
        "backend": "hf_adapter",
        "prototype_backend": "triton_int8_rowwise_gemv" if native_int8_gemv_available() else "torch_fallback",
        "status": "pass",
