#!/usr/bin/env python3
# coding=utf-8
"""Benchmark optional fused fp16 W/A/G LoRA projection prototype."""
...
from rwkv7_hf.fused_lora import fused_wag_lora, fused_wag_lora_available
...
def current_wag(attn, xs: dict[str, torch.Tensor]):
    return attn.w_lora(xs["xw"]), attn.a_lora(xs["xa"]), attn.g_lora(xs["xg"])


def prototype_wag(attn, xs: dict[str, torch.Tensor], block_m: int, block_r: int, block_k: int):
    return fused_wag_lora(
        xs["xw"],
        xs["xa"],
        xs["xg"],
...
def main() -> int:
    ...
    for layer_idx in args.layers:
        ...
        with torch.inference_mode():
            cur = current_wag(attn, xs)
            proto = prototype_wag(attn, xs, args.block_m, args.block_r, args.block_k)
        current_ms = timed(lambda: current_wag(attn, xs), args.device, args.warmup, args.steps)
        prototype_ms = timed(lambda: prototype_wag(attn, xs, args.block_m, args.block_r, args.block_k), args.device, args.warmup, args.steps)
        layer_rows.append(
            {
                "layer_idx": layer_idx,
                "ranks": layer_ranks,
    ...
    row = {
        ...
        "backend": "hf_adapter",
        "prototype_backend": "triton_fused_wag_lora" if fused_wag_lora_available() else "torch_fallback",
        "status": "pass",
        "dtype": args.dtype,
