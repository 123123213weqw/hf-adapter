#!/usr/bin/env python3
# coding=utf-8
"""A/B benchmark for native_graph with fused W/A/G LoRA enabled.

`bench_fused_wag_lora.py` shows the isolated W/A/G LoRA grouping can be
profitable. This script checks the production-facing question: after capture
inside the HF native_graph fast-token backend, does enabling
`RWKV7_NATIVE_GRAPH_FUSED_WAG_LORA=1` preserve logits/greedy behavior and move
end-to-end decode latency on top of the current output-prep default?
"""
...
def run_mode(model, token: torch.Tensor, base_state, args: argparse.Namespace, *, enabled: bool) -> dict[str, Any]:
    ...
    os.environ["RWKV7_NATIVE_GRAPH_FUSED_WAG_LORA"] = "1" if enabled else "0"
...
def main() -> int:
    ...
    ap.add_argument("--fused-output", action=argparse.BooleanOptionalAction, default=True, help="Keep default fused output prep enabled in both A/B modes.")
    ...
    ap.add_argument("--fused-recurrent", action="store_true", help="Keep RWKV7_NATIVE_GRAPH_FUSED_RECURRENT=1 in both A/B modes to test combined recurrent/WAG-LoRA integration.")
    ap.add_argument("--block-m", type=int, default=64, help="Fused W/A/G LoRA output tile.")
    ap.add_argument("--block-r", type=int, default=64, help="Fused W/A/G LoRA rank tile.")
    ap.add_argument("--block-k", type=int, default=64, help="Fused W/A/G LoRA hidden tile.")
