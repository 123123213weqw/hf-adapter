#!/usr/bin/env python3
# coding=utf-8
"""A/B benchmark for native_graph with the fused recurrent update enabled.

`bench_fused_recurrent.py` proves the isolated rank-1 recurrent kernel is fast.
This script checks the production-facing question: after capture inside the HF
native_graph fast-token backend, does enabling
`RWKV7_NATIVE_GRAPH_FUSED_RECURRENT=1` preserve logits/greedy behavior and move
