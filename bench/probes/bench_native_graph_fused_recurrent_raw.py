#!/usr/bin/env python3
# coding=utf-8
"""End-to-end A/B for raw recurrent-output preparation fusion.

The benchmark changes only ``RWKV7_NATIVE_GRAPH_FUSED_RECURRENT_RAW`` between
captures. Existing output/recurrent-output, norm/mix, LoRA and projection
routes remain unchanged, so the result measures the incremental benefit of
folding W decay and K/KK preparation into the recurrent kernel.
It records first-step logits, a multi-step greedy window, graph-cache
telemetry, latency, throughput, and peak allocated VRAM.
"""
...
def run_mode(model, token: torch.Tensor, base_state, args: argparse.Namespace, *, enabled: bool) -> dict[str, Any]:
    os.environ["RWKV7_NATIVE_GRAPH_FUSED_RECURRENT_RAW"] = "1" if enabled else "0"
    ...
    with torch.inference_mode():
        ...
        for _ in range(args.steps):
            ...
            if not args.fixed_token:
                ...
                tok = token
            greedy_tokens.extend(
                int(v) for v in out.logits[:, -1, :].argmax(dim=-1).detach().cpu().reshape(-1)
            )
        cuda_sync(args.device)
...
def main() -> int:
    ...
    row = {
        "axis": "native_graph_fused_recurrent_raw",
        "backend": "hf_adapter",
