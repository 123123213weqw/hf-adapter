#!/usr/bin/env bash
# Reproduce and gate all 36 exact RTX 4090 parameter-adjusted P/D cells.
set -euo pipefail
...
# These are the two exact runtimes used by the committed matrix.  Do not copy
# the RTX 4080 CUDA 13 candidate stack onto driver-550 systems.
"${RWKV_PYTHON_BIN}" - <<'PY'
...
run_matrix() {
  ...
  "${python_bin}" bench/bench_cross_model_speed_resident.py \
    ...
    --benchmark-matrix qwen35_4090_adjusted_pd --dtype fp16 --quantization none \
...
# Use the exact-card policy as the single source of truth.  In particular,
# B8 grouped W/A/V BMM and scoped FP16 accumulation must be selected by the
# committed 4090 allowlists rather than benchmark-only overrides.
