#!/usr/bin/env bash
# Capture the strict RTX 4080 RWKV candidate matrix and FLA correctness oracle.
set -euo pipefail
...
for model in "${RWKV_04_MODEL}" "${RWKV_15_MODEL}" "${RWKV_29_MODEL}"; do
...
for protected in "${ROOT}" "${RWKV_04_MODEL}" "${RWKV_15_MODEL}" "${RWKV_29_MODEL}"; do
...
expected = {"python":"3.12.2","torch":"2.11.0+cu130","torch_cuda":"13.0","triton":"3.6.0","transformers":"5.12.1","fla":"0.5.1","causal_conv1d":"1.6.2.post1"}
if runtime != expected:
...
hash_models() {
  local output="$1"
  env -i "${COMMON_ENV[@]}" "${PYTHON_BIN}" - "${output}" "${RWKV_04_MODEL}" "${RWKV_15_MODEL}" "${RWKV_29_MODEL}" <<'PY'
...
for spec in \
  ...
  "2p9|rwkv-2.9b__qwen3.5-4b|2.9b|${RWKV_29_MODEL}"; do
...
specs=[("0p4","rwkv-0.4b__qwen3.5-0.8b","0.4b"),("1p5","rwkv-1.5b__qwen3.5-2b","1.5b"),("2p9","rwkv-2.9b__qwen3.5-4b","2.9b")]
...
correctness.write_text(json.dumps({"schema_version":1,"protocol":"rwkv_native_graph_fla_correctness_4080_v1","benchmark_repository_commit":commit,"model_hashes_sha256":sha(model_hashes),"runtime":artifact(runtime),"coverage":{"models":3,"batch_sizes":[1,8],"entries":6,"baseline_fresh_gpu_processes":6,"candidate_additional_gpu_processes":0,"candidate_formal_lane_processes":6,"prompt_tokens":2048,"decode_tokens":512,"probe_tokens":512},"gates":{"greedy_tokens":"exact_all_512","prompt_logits_min_row_cosine":0.9999,"final_logits_min_row_cosine":0.9999,"decode_logits_all_finite":True,"b8_distinct_prompts":True},"entries":entries},indent=2)+"\n")
