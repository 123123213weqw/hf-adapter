...
from rwkv7_hf.ascend_w4_cle import (
...
def test_sqrelu_channel_equalization_is_mathematically_exact():
    torch.manual_seed(4)
    key = nn.Linear(8, 16, bias=True, dtype=torch.float64)
    value = nn.Linear(16, 8, bias=False, dtype=torch.float64)
...
def test_cle_grid_never_worse_than_identity_candidate():
    torch.manual_seed(5)
    key = nn.Linear(8, 16, bias=False, dtype=torch.float32)
    value = nn.Linear(16, 8, bias=False, dtype=torch.float32)
    x = torch.randn(64, 8)
    baseline = calibrate_sqrelu_value_w4(key, value, x, group_size=4, alphas=())
    chosen = calibrate_sqrelu_value_w4(
        key, value, x, group_size=4, alphas=(0.0, 0.25, 0.5, 0.75, 1.0)
    )
