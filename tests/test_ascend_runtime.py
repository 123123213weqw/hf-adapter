...
def test_exact_stack_validation_has_no_substring_or_version_family_matching():
    from rwkv7_hf.ascend_runtime import (
        normalize_ascend_device_name,
...
def _patch_fake_available_runtime(
    monkeypatch, runtime, *, device_name, torch_npu_version="2.9.0"
):
    import torch
    ...
    monkeypatch.setattr(
        runtime, "import_torch_npu", lambda required=False: SimpleNamespace()
    )
...
def test_enable_ascend_rejects_unknown_card_before_set_device(monkeypatch):
    ...
    with pytest.raises(
        RuntimeError, match="unvalidated Huawei Ascend production stack"
    ):
...
def test_enable_ascend_explicit_override_is_reported_unvalidated(monkeypatch):
    ...
    assert (
        "device_name" in info.validation_reason
        and "torch_npu_version" in info.validation_reason
    )
    assert selected == ["npu:0"]
