...
def main() -> int:
    ...
    ap.add_argument(
        "--backend",
        choices=["eager", "native_jit", "native_graph", "auto"],
        default="eager",
    )
    ...
    with torch.inference_mode():
        ...
        assert (
            torch.isfinite(full.logits).all() and torch.isfinite(chunked.logits).all()
        )
        assert torch.allclose(
            full.logits.float(), chunked.logits.float(), rtol=8e-3, atol=8e-2
        )
        cache = chunked.past_key_values
        assert isinstance(cache, NativeRWKV7Cache) and cache.seen_tokens == ids.shape[1]
        selected = cache.clone().select_batch(
            torch.tensor([1], device=args.device), inplace=True
        )
        assert selected.get_batch_size() == 1
        step = model(
            torch.tensor([[13]], device=args.device),
    ...
    if args.training_smoke:
        ...
        assert (
            train_out.loss is not None
            and torch.isfinite(train_out.loss).detach().cpu().item()
        )
        train_out.loss.backward()
        grads = [p.grad for p in model.parameters() if p.grad is not None]
        assert grads and all(
            torch.isfinite(g).all().detach().cpu().item() for g in grads
        )
        parameters = [
            p
            for p in model.parameters()
            if p.grad is not None and torch.count_nonzero(p.grad).detach().cpu().item()
        ]
        assert parameters
        parameter = max(
            parameters, key=lambda p: float(p.grad.detach().abs().max().cpu())
        )
    ...
    if args.model:
        ...
        encoded = (
            tokenizer("Ascend", return_tensors="pt").input_ids[:, :4].to(args.device)
        )
        with torch.inference_mode():
        ...
        with tempfile.TemporaryDirectory(prefix="rwkv7-ascend-roundtrip-") as temp:
            ...
            reloaded = (
                NativeRWKV7ForCausalLM.from_pretrained(temp, torch_dtype=dtype)
                .eval()
                .to(args.device)
            )
            with torch.inference_mode():
            ...
            assert torch.allclose(
                out.logits.float(), again.logits.float(), rtol=1e-3, atol=1e-2
            )
        auto = {
            "auto_config": acfg.__class__.__name__,
            "auto_model": base.__class__.__name__,
            "auto_causal_lm": loaded.__class__.__name__,
            "auto_tokenizer": tokenizer.__class__.__name__,
            "save_reload": "pass",
            "auto_generate": list(gen.shape),
        }

    row = {
        "axis": "huawei_ascend_hf_acceptance",
        "status": "pass",
        **info.to_dict(),
        "dtype": args.dtype,
        "forward": "pass",
