...
def test_speed_policy_is_exact_shape_role_card_and_rows(monkeypatch):
    ...
    assert (
        candidate.enabled
        and not candidate.speed_validated
        and candidate.stack_validated
    )
    assert not ascend_w8a16_decision(
        "model.layers.0.att.key",
        4096,
        16384,
        device_name="Ascend910B3",
        rows=8,
    ...
    spoofed = ascend_w8a16_decision(
        ...
        16384,
        4096,
        **{**exact_stack, "device_name": "FakeAscend910B3X"},
        policy="candidate",
    )
    ...
    wrong_stack = ascend_w8a16_decision(
        ...
        16384,
        4096,
        **{**exact_stack, "torch_npu_version": "2.9.0.post1"},
        policy="candidate",
    )
...
def test_w4_candidate_is_explicit_and_packed(tmp_path):
    ...
    assert (
        quant.packed_weight_bytes() < dense.weight.numel() * dense.weight.element_size()
    )
